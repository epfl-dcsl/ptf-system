...
class QueueRunner(object):
  ...
  def __init__(self, queue=None, enqueue_ops=None, close_op=None,
               ...
               queue_runner_def=None, import_scope=None, device=None):
    ...
    if queue_runner_def:
      ...
      self._init_from_args(
          ...
          queue_closed_exception_types=queue_closed_exception_types, device=device)
  ...
  def _init_from_args(self, queue=None, enqueue_ops=None, close_op=None,
                      cancel_op=None, queue_closed_exception_types=None, device=None):
    ...
    if device is None:
        device_count = defaultdict(lambda: 0)
        for enq_op_input in itertools.chain.from_iterable(enq_op.inputs[1:] for enq_op in enqueue_ops):
          device_count[enq_op_input.device] += 1
        assert len(device_count) > 0
        if len(device_count) > 1:
          print("QueueRunner {name} has >1 device ({devices}). Choosing the most popular one. Specify using constructor otherwise".format(
            name=self.name, devices=list(device_count.keys())
          ))
        device = sorted(device_count.items(), key=itemgetter(1), reverse=True)[0][0]
  ...
  def _init_from_proto(self, queue_runner_def, import_scope=None):
    ...
    self._device = queue_runner_def.device
    self._queue_closed_exception_types = tuple(
  ...
  def create_threads(self, sess, coord=None, daemon=False, start=False):
    ...
    for op in self._enqueue_ops:
      name = "QueueRunnerThread-{}-{}".format(self.smart_name, op.name)
      ret_threads.append(threading.Thread(target=self._run,
                                          args=(sess, op, coord),
                                          name=name))
    if coord:
      name = "QueueRunnerThread-{}-close_on_stop".format(self.smart_name)
  ...
  def to_proto(self, export_scope=None):
    ...
    if (export_scope is None or
      ...
      queue_runner_def.device = self.device
...
def start_queue_runners(sess=None, coord=None, daemon=True, start=True,
                        collection=ops.GraphKeys.QUEUE_RUNNERS, device=None):
  ...
  with sess.graph.as_default():
    ...
    for qr in ops.get_collection(collection):
      if device is None or qr.device == device:
