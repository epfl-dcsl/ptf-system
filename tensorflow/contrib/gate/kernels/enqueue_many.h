...
namespace tensorflow {
  ...
  class GateEnqueueManyOp : public GateAccessOp<GateType> {
  public:
    explicit GateEnqueueManyOp(OpKernelConstruction *ctx) : GateAccessOp<GateType>(ctx) {
  ...
  protected:
    void ComputeWithGate(OpKernelContext *ctx, GateType *barrier, GateInterface::DoneCallback done) override {
        ...
        OP_REQUIRES_OK_ASYNC(ctx, barrier->ValidateManyTuple(t), done);

        barrier->TryEnqueueMany(*id_and_count_t, t, ctx, done);
    }
