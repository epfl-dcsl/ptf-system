...
namespace tensorflow {
  ...
  class GateDequeueManyOp : public GateAccessOp<GateType> {
  public:
    explicit GateDequeueManyOp(OpKernelConstruction *ctx) : GateAccessOp<GateType>(ctx) {
  ...
  protected:
    void ComputeWithGate(OpKernelContext *ctx, GateType *barrier, GateInterface::DoneCallback done) override {
      barrier->TryDequeueMany(batch_size_, ctx, [ctx, done](const Tensor &id_and_count, const typename GateType::Tuple &tuple) {
        if (!ctx->status().ok()) {
          done();
          return;
        }
