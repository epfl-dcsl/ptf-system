...
namespace tensorflow {
  ...
  public:
    AGDReaderOp(OpKernelConstruction *context) : OpKernel(context) {
      ...
      OP_REQUIRES_OK(context, context->GetAttr("repack", &repack_));
      DCHECK(not (unpack_ and repack_)); // shape function should check this
    }

    ~AGDReaderOp() override {
      ...
      core::ScopedUnref unref_me(agd_input);
      {
        ...
        auto input_data = agd_input->get();
        {
          ...
          OP_REQUIRES_OK(ctx, rec_parser_.ParseNew(input_data->data(), input_data->size(),
                                                   verify_, output_ptr, &first_ord_, &num_recs_, record_id_, unpack_, repack_));
  ...
  private:
    bool verify_ = false, unpack_ = true, repack_ = false;
