...
namespace tensorflow {
  ...
  public:
    ...
    void Compute(OpKernelContext* ctx) override {
      ...
      auto compress_duration = chrono::duration_cast<chrono::microseconds>(end - start);
      auto prior_bytes = buf_pair->data().size() + buf_pair->index().size();
      auto new_bytes = buf->size();
        Tensor *stamp_t, *duration_t, *bytes_t, *compressed_t;
        ...
        OP_REQUIRES_OK(ctx, ctx->allocate_output("original_size", TensorShape({}), &bytes_t));
        OP_REQUIRES_OK(ctx, ctx->allocate_output("compressed_size", TensorShape({}), &compressed_t));
        stamp_t->scalar<int64>()() = chrono::duration_cast<chrono::microseconds>(start.time_since_epoch()).count();
        duration_t->scalar<int64>()() = compress_duration.count();
        bytes_t->scalar<int32>()() = prior_bytes;
        compressed_t->scalar<int32>()() = new_bytes;
  ...
  private:
    ...
    Status AppendRecord(const T* data, unsigned elements_t, BufferPair &bp) {
      ...
      if (not (index.AppendBuffer(reinterpret_cast<char*>(&converted_size), sizeof(converted_size)) and data_buf.AppendBuffer(converted_data, converted_size))) {
        return Internal("Unable to append into index or data");
      }
