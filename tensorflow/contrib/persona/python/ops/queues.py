...
def _enqueue_join(queue, tensor_list_list, enqueue_many, control_dependencies):
  """Enqueue `tensor_list_list` in `queue`."""
  assert len(tensor_list_list) == len(control_dependencies)
  if enqueue_many:
    enqueue_fn = queue.enqueue_many
  else:
    enqueue_fn = queue.enqueue
  def gen_enq_ops():
    for tl, cd in zip(tensor_list_list, control_dependencies):
      with ops.control_dependencies(cd):
        yield enqueue_fn(tl)
  enqueue_ops = tuple(gen_enq_ops())
  queue_runner.add_queue_runner(queue_runner.QueueRunner(queue, enqueue_ops))


def _enqueue(queue, tensor_list, threads, enqueue_many, control_dependencies):
  """Enqueue `tensor_list` in `queue`."""
  assert len(tensor_list) == len(control_dependencies)
  if enqueue_many:
    enqueue_fn = queue.enqueue_many
  else:
    enqueue_fn = queue.enqueue
  def gen_enq_ops():
    for tl, cd in zip(tensor_list, control_dependencies):
      with ops.control_dependencies(cd):
        yield enqueue_fn(tl)
  enqueue_ops = tuple(gen_enq_ops()) * threads
  queue_runner.add_queue_runner(queue_runner.QueueRunner(queue, enqueue_ops))
...
def _prepare_control_deps(num_groups, control_deps):
  if control_deps is None:
    return tuple(itertools.repeat([], times=num_groups))
  else:
    try:
      control_deps[0]
    except TypeError:
      control_deps = tuple(control_deps)
    if len(control_deps) != num_groups:
      raise Exception("Got {cd} control dependencies, but only {actual} groups. They must be equal!".format(
        cd=len(control_deps), actual=num_groups
      ))
    return control_deps

def batch_pdq(tensor_list, batch_size, num_threads=1, capacity=32, num_dq_ops=1,
              enqueue_many=False, shapes=None, dynamic_pad=False,
              shared_name=None, name=None, control_dependencies=None):
  ...
  *N.B.:* If `dynamic_pad` is `False`, you must ensure that either
  ...
  control_dependencies = _prepare_control_deps(num_groups=len(tensor_list), control_deps=control_dependencies)
  ...
  with ops.name_scope(name, "batch_pdq", tensor_list) as name:
    ...
    _enqueue(queue, tensor_list, num_threads, enqueue_many, control_dependencies=control_dependencies)
    summary.scalar(
...
def batch_join_pdq(tensor_list_list, batch_size, num_dq_ops=1, capacity=32,
  ...
  *N.B.:* If `dynamic_pad` is `False`, you must ensure that either
  ...
  """
  control_dependencies = _prepare_control_deps(num_groups=len(tensor_list_list), control_deps=control_dependencies)
  ...
  with ops.name_scope(name, "batch_join_pdq",_flatten(tensor_list_list)) as name:
    ...
    queue = _which_queue(dynamic_pad)(
        capacity=capacity, dtypes=types, shapes=shapes, shared_name=shared_name)
    _enqueue_join(queue, tensor_list_list, enqueue_many, control_dependencies=control_dependencies)
    summary.scalar(
        "queue/%s/fraction_of_%d_full" % (queue.name, capacity),
        math_ops.cast(queue.size(), dtypes.float32) * (1. / capacity))
